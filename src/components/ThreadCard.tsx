...
const ThreadCard: React.FC<ThreadCardProps> = ({ thread }) => {
    ...
    return (
        <button onClick={() => navigate(`/threads/${thread.title.replace(/\s/g, "-")}.${thread.id}`)} key={thread.id} className="flex flex-col text-left md:min-w-[35rem] bg-zinc-300 hover:bg-light-secondary dark:bg-zinc-900 hover:dark:bg-dark-secondary p-4 gap-4 border border-zinc-400 dark:border-zinc-800 rounded-md">
